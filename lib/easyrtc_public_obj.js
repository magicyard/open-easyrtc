...
pub.getAppWithEasyrtcid = function (easyrtcid, callback) {
  ...
  callback(
    new pub.util.ConnectionWarning(
      "Can not find connection [" + easyrtcid + "]"
    )
  );
};
...
pub.getOption = function (optionName) {
  if (typeof e.option[optionName] === "undefined") {
    pub.util.logError(
      "Unknown option requested. Unrecognised option name '" + optionName + "'."
    );
    return null;
  }
...
pub.setOption = function (optionName, optionValue) {
  ...
  if (e.option.hasOwnProperty(optionName)) {
    ...
    pub.util.logError(
      "Error setting option. Unrecognised option name '" + optionName + "'."
    );
    return false;
  }
};
 ...
 */
pub.events.emit = pub.events._eventListener.emit.bind(
  pub.events._eventListener
);
...
pub.events.emitDefault = function () {
  if (!pub.events.defaultListeners[arguments["0"]]) {
    console.error(
      "Error emitting listener. No default for event '" +
        arguments["0"] +
        "' exists."
    );
    return;
  }
  pub.events.defaultListeners[Array.prototype.shift.call(arguments)].apply(
    this,
    arguments
  );
};
...
pub.events.setDefaultListener = function (eventName) {
  if (!_.isFunction(pub.events.defaultListeners[eventName])) {
    console.error(
      "Error setting default listener. No default for event '" +
        eventName +
        "' exists."
    );
  }
  pub.events._eventListener.removeAllListeners(eventName);
  pub.events._eventListener.on(
    eventName,
    pub.events.defaultListeners[eventName]
  );
};
...
pub.events.setDefaultListeners = function () {
  ...
  for (var currentEventName in pub.events.defaultListeners) {
    if (_.isFunction(pub.events.defaultListeners[currentEventName])) {
      pub.events._eventListener.on(
        currentEventName,
        pub.events.defaultListeners[currentEventName]
      );
    } else {
      throw new pub.util.ServerError(
        "Error setting default listener. No default for event '" +
          currentEventName +
          "' exists."
      );
...
pub.util.getErrorMsg = function (errorCode) {
  ...
  if (!msg.msgData.errorText) {
    ...
    pub.util.logWarning(
      "Emitted unknown error with error code [" + errorCode + "]"
    );
  }
...
var errorCodesToMessages = {
  ...
  LOGIN_APP_AUTH_FAIL:
    "Authentication for application failed. Socket will be disconnected.",
  LOGIN_BAD_APP_NAME:
    "Provided application name is improper. Socket will be disconnected.",
  LOGIN_BAD_AUTH:
    "Authentication for application failed. Socket will be disconnected.",
  LOGIN_BAD_ROOM:
    "Requested room is invalid or does not exist. Socket will be disconnected.",
  LOGIN_BAD_STRUCTURE:
    "Authentication for application failed. The provided structure is improper. Socket will be disconnected.",
  LOGIN_BAD_USER_CFG:
    "Provided configuration options improper or invalid. Socket will be disconnected.",
  LOGIN_GEN_FAIL: "Authentication failed. Socket will be disconnected.",
  LOGIN_NO_SOCKETS:
    "No sockets available for account. Socket will be disconnected.",
  ...
  MSG_REJECT_BAD_ROOM:
    "Message rejected. Requested room is invalid or does not exist.",
  MSG_REJECT_BAD_FIELD:
    "Message rejected. Problem with field structure or name.",
  MSG_REJECT_BAD_SIZE: "Message rejected. Packet size is too large.",
  MSG_REJECT_BAD_STRUCTURE:
    "Message rejected. The provided structure is improper.",
  ...
  MSG_REJECT_NO_AUTH:
    "Message rejected. Not logged in or client not authorized.",
  ...
  MSG_REJECT_TARGET_EASYRTCID:
    "Message rejected. Target easyrtcid is invalid, not using same application, or no longer online.",
  MSG_REJECT_TARGET_GROUP:
    "Message rejected. Target group is invalid or not defined.",
  MSG_REJECT_TARGET_ROOM:
    "Message rejected. Target room is invalid or not created.",
...
pub.util.getErrorText = function (errorCode) {
  if (errorCodesToMessages.hasOwnProperty(errorCode)) {
    ...
    pub.util.logWarning(
      "Unknown message errorCode requested [" + errorCode + "]"
    );
    return null;
  }
};
...
pub.util.sendSocketCallbackAck = function (easyrtcid, socketCallback, appObj) {
  return pub.util.sendSocketCallbackMsg(
    easyrtcid,
    socketCallback,
    { msgType: "ack" },
    appObj
 ...
 * @param       {?Object}   appObj      EasyRTC application object. Contains methods used for identifying and managing an application.
 */
pub.util.sendSocketCallbackMsg = function (
  easyrtcid,
  socketCallback,
  msg,
  appObj
    ...
    if (!appObj.isConnectedSync(easyrtcid)) {
      pub.util.logDebug(
        "[" +
          ...
          "][" +
          easyrtcid +
          "] Unable to return socket message. Peer no longer connected."
      );
      return false;
    }
  }
  ...
  if (!_.isFunction(socketCallback)) {
    pub.util.logWarning(
      "[" +
        ...
        "][" +
        easyrtcid +
        "] Unable to return socket message. Provided socketCallback was not a function."
    );
    return false;
  }
  ...
  try {
    ...
    pub.util.logWarning(
      "[" +
        ...
        "][" +
        easyrtcid +
        "] Unable to return socket message. Call to socketCallback failed."
    );
  }
    ...
    try {
      pub.util.logDebug(
        "[" +
          ...
          "][" +
          easyrtcid +
          "] Returning socket.io message: [" +
          JSON.stringify(msg) +
          "]"
      );
    } catch (err) {
      pub.util.logDebug(
        "[" + appName + "][" + easyrtcid + "] Returning socket.io message"
      );
    }
  }
  return true;
...
pub.util.isValidIncomingMessage = function (type, msg, appObj, callback) {
  ...
  switch (type) {
    case "easyrtcAuth":
      ...
      if (
        msg.msgData.apiVersion === undefined ||
      ...
      // msgData.appName
      if (
        msg.msgData.applicationName !== undefined &&
        (!_.isString(msg.msgData.applicationName) ||
          !getOption("appNameRegExp").test(msg.msgData.applicationName))
      ) {
        callback(null, false, "MSG_REJECT_BAD_STRUCTURE");
        return;
      }
      ...
      // msgData.easyrtcsid
      if (
        msg.msgData.easyrtcsid !== undefined &&
        (!_.isString(msg.msgData.easyrtcsid) ||
          !getOption("easyrtcsidRegExp").test(msg.msgData.easyrtcsid))
      ) {
        callback(null, false, "MSG_REJECT_BAD_STRUCTURE");
        return;
      }
      ...
      async.waterfall(
        [
          ...
          function (asyncCallback) {
            // msgData.username
            if (
              msg.msgData.username !== undefined &&
              (!_.isString(msg.msgData.username) ||
                !getOption("usernameRegExp").test(msg.msgData.username))
            ) {
              callback(null, false, "MSG_REJECT_BAD_STRUCTURE");
              isCallbackRun = true;
              return;
            ...
            // msgData.credential
            if (
              msg.msgData.credential !== undefined &&
              (!_.isObject(msg.msgData.credential) ||
                _.isEmpty(msg.msgData.credential))
            ) {
              callback(null, false, "MSG_REJECT_BAD_STRUCTURE");
              isCallbackRun = true;
              return;
              ...
              for (var currentRoomName in msg.msgData.roomJoin) {
                if (msg.msgData.roomJoin.hasOwnProperty(currentRoomName)) {
                  if (
                    ...
                    !_.isString(
                      msg.msgData.roomJoin[currentRoomName].roomName
                    ) ||
                    currentRoomName !==
                      msg.msgData.roomJoin[currentRoomName].roomName
                  ) {
            ...
            if (msg.msgData.setPresence !== undefined) {
              if (
                ...
                _.isEmpty(msg.msgData.setPresence)
              ) {
                callback(null, false, "MSG_REJECT_BAD_STRUCTURE");
                isCallbackRun = true;
                return;
              ...
              if (
                msg.msgData.setPresence.show !== undefined &&
                (!_.isString(msg.msgData.setPresence.show) ||
                  !getOption("presenceShowRegExp").test(
                    msg.msgData.setPresence.show
                  ))
              ) {
                callback(null, false, "MSG_REJECT_BAD_STRUCTURE");
                isCallbackRun = true;
              ...
              if (
                msg.msgData.setPresence.status !== undefined &&
                (!_.isString(msg.msgData.setPresence.status) ||
                  !getOption("presenceStatusRegExp").test(
                    msg.msgData.setPresence.status
                  ))
              ) {
                callback(null, false, "MSG_REJECT_BAD_STRUCTURE");
                isCallbackRun = true;
    ...
    case "easyrtcCmd":
      switch (msg.msgType) {
        ...
        case "answer":
          ...
          if (
            !_.isString(msg.targetEasyrtcid) ||
            !getOption("easyrtcidRegExp").test(msg.targetEasyrtcid)
          ) {
            callback(null, false, "MSG_REJECT_BAD_STRUCTURE");
            return;
          }
        ...
        case "setRoomApiField":
          ...
          if (
            !_.isObject(msg.msgData.setRoomApiField) ||
            _.isEmpty(msg.msgData.setRoomApiField)
          ) {
            callback(null, false, "MSG_REJECT_BAD_STRUCTURE");
            return;
          }
          if (
            !_.isString(msg.msgData.setRoomApiField.roomName) ||
            !getOption("roomNameRegExp").test(
              msg.msgData.setRoomApiField.roomName
            )
          ) {
          ...
          if (msg.msgData.setRoomApiField.field !== undefined) {
            ...
            try {
              if (
                JSON.stringify(msg.msgData.setRoomApiField.field).length >= 4096
              ) {
        ...
        case "setUserCfg":
          ...
          if (
            !_.isObject(msg.msgData.setUserCfg) ||
            _.isEmpty(msg.msgData.setUserCfg)
          ) {
            callback(null, false, "MSG_REJECT_BAD_STRUCTURE");
            return;
          }
          ...
          // setUserCfg.p2pList
          if (
            msg.msgData.setUserCfg.p2pList !== undefined &&
            (!_.isObject(msg.msgData.setUserCfg.p2pList) ||
              _.isEmpty(msg.msgData.setUserCfg.p2pList))
          ) {
            callback(null, false, "MSG_REJECT_BAD_STRUCTURE");
            return;
          }
          ...
          if (
            msg.msgData.setUserCfg.userSettings !== undefined &&
            (!_.isObject(msg.msgData.setUserCfg.userSettings) ||
              _.isEmpty(msg.msgData.setUserCfg.userSettings))
          ) {
            callback(null, false, "MSG_REJECT_BAD_STRUCTURE");
            return;
          }
    ...
    case "easyrtcMsg":
      ...
      // targetGroup
      if (
        msg.targetGroup !== undefined &&
        (!_.isString(msg.targetGroup) ||
          !getOption("groupNameRegExp").test(msg.targetGroup))
      ) {
      ...
      // targetRoom
      if (
        msg.targetRoom !== undefined &&
        (!_.isString(msg.targetRoom) ||
          !getOption("roomNameRegExp").test(msg.targetRoom))
      ) {
...
pub.util.sendSessionCookie = function (req, res) {
  ...
  if (
    !pub.getOption("sessionEnable") ||
    !pub.getOption("sessionCookieEnable")
  ) {
    return;
  }
  if (
    req.sessionID &&
    (!req.cookies ||
      !req.cookies.easyrtcsid ||
      req.cookieseasyrtcsid !== req.sessionID)
  ) {
    try {
      pub.util.logDebug(
        "Sending easyrtcsid cookie [" +
          req.sessionID +
          "] to [" +
          req.ip +
          "] for request [" +
      ...
      pub.util.logWarning(
        "Problem setting easyrtcsid cookie [" +
          req.sessionID +
          "] to [" +
          req.ip +
          "] for request [" +
...
pub.createApp = function (appName, options, callback) {
  ...
  if (!appName || !pub.getOption("appNameRegExp").test(appName)) {
    pub.util.logWarning(
      "Can not create application with improper name: '" + appName + "'"
    );
    callback(
      new pub.util.ApplicationWarning(
        "Can not create application with improper name: '" + appName + "'"
      )
    );
    return;
  ...
  if (e.app[appName]) {
    pub.util.logWarning(
      "Can not create application which already exists: '" + appName + "'"
    );
    callback(
      new pub.util.ApplicationWarning(
        "Can not create application which already exists: '" + appName + "'"
  ...
  pub.app(appName, function (err, appObj) {
    ...
    async.each(
      ...
      function (err) {
        if (err) {
          callback(
            new pub.util.ApplicationError(
              "Could not set options when creating application: '" +
                appName +
                "'",
              err
            )
        ...
        if (appObj.getOption("roomDefaultEnable")) {
          pub.events.emit(
            ...
            appObj,
            null,
            appObj.getOption("roomDefaultName"),
            null,
...
pub.app = function (appName, callback) {
  ...
  if (!e.app[appName]) {
    pub.util.logDebug(
      "Attempt to request non-existent application name: '" + appName + "'"
    );
    callback(
      new pub.util.ApplicationWarning(
        "Attempt to request non-existent application name: '" + appName + "'"
  ...
  appObj.getField = function (fieldName, callback) {
    if (!e.app[appName].field[fieldName]) {
      ...
      callback(
        new pub.util.ApplicationWarning(
          "Can not find app field: '" + fieldName + "'"
        )
      );
      return;
    }
  ...
  appObj.getFields = function (limitToIsShared, callback) {
    ...
    for (var fieldName in e.app[appName].field) {
      if (
        !limitToIsShared ||
        e.app[appName].field[fieldName].fieldOption.isShared
      ) {
        fieldObj[fieldName] = {
          fieldName: fieldName,
          fieldValue: pub.util.deepCopy(
            e.app[appName].field[fieldName].fieldValue
          ),
        };
      }
    }
  ...
  appObj.setField = function (fieldName, fieldValue, fieldOption, next) {
    pub.util.logDebug(
      "[" + appName + "] Setting field [" + fieldName + "]",
      fieldValue
    );
    ...
    if (!pub.getOption("fieldNameRegExp").test(fieldName)) {
      pub.util.logWarning(
        "Can not create application field with improper name: '" +
          fieldName +
          "'"
      );
      next(
    ...
    e.app[appName].field[fieldName] = {
      ...
      fieldOption: {
        isShared:
          _.isObject(fieldOption) && fieldOption.isShared ? true : false,
      },
    };
  ...
  appObj.connection = function (easyrtcid, callback) {
    if (!e.app[appName].connection[easyrtcid]) {
      pub.util.logWarning(
        "Attempt to request non-existent connection key: '" + easyrtcid + "'"
      );
      callback(
        new pub.util.ConnectionWarning(
          "Attempt to request non-existent connection key: '" + easyrtcid + "'"
    ...
    if (!pub.socketServer) {
      ...
      callback(
        new pub.util.ConnectionWarning(
          "Attempt to request non-existent socket: '" + easyrtcid + "'"
        )
      );
      return;
    }
    ...
    if (pub.socketServer.sockets.sockets) {
      if (!pub.socketServer.sockets.sockets.get(socketId)) {
        pub.util.logWarning(
          "[" +
            easyrtcid +
            "] Attempt to request non-existent socket: '" +
            socketId +
            "'"
        );
        callback(
          new pub.util.ConnectionWarning(
            "Attempt to request non-existent socket: '" + socketId + "'"
          )
        );
        return;
      }
      ...
      if (pub.socketServer.sockets.sockets.get(socketId).disconnected) {
        pub.util.logWarning(
          "[" +
            easyrtcid +
            "] Attempt to request disconnected socket: '" +
            socketId +
            "'"
        );
        callback(
          new pub.util.ConnectionWarning(
            "Attempt to request disconnected socket: '" + socketId + "'"
          )
        );
        return;
      }
    ...
    connectionObj.getField = function (fieldName, callback) {
      if (
        ...
        callback(
          new pub.util.ApplicationWarning(
            "Can not find connection field: '" + fieldName + "'"
          )
        );
        return;
      }
    ...
    connectionObj.getFieldValueSync = function (fieldName) {
      if (
        ...
        return pub.util.deepCopy(
          e.app[appName].connection[easyrtcid].field[fieldName].fieldValue
        );
      } else {
        return null;
      }
    ...
    connectionObj.getFields = function (limitToIsShared, callback) {
      ...
      if (e.app[appName].connection[easyrtcid]) {
        for (var fieldName in e.app[appName].connection[easyrtcid].field) {
          ...
          if (
            !limitToIsShared ||
            e.app[appName].connection[easyrtcid].field[fieldName].fieldOption
              .isShared
          ) {
            fieldObj[fieldName] = {
              fieldName: fieldName,
    ...
    connectionObj.joinSession = function (easyrtcsid, next) {
        ...
        next(
          new pub.util.ConnectionWarning(
            "[" +
              ...
              "] Session [" +
              easyrtcsid +
              "] does not exist. Could not join session"
          )
        );
        return;
      }
      ...
      appObj.session(easyrtcsid, function (err, sessionObj) {
        ...
        e.app[appName].connection[easyrtcid].toSession =
          e.app[appName].session[easyrtcsid];
        e.app[appName].connection[easyrtcid].toSession.toConnection[easyrtcid] =
          e.app[appName].connection[easyrtcid];
        ...
        next(
          new pub.util.ConnectionWarning(
            "[" +
              ...
              "] Connection [" +
              easyrtcid +
              "] does not exist. Could not authenticate"
          )
        );
        return;
      }
     ...
     */
    connectionObj.setField = function (
      fieldName,
      fieldValue,
      fieldOption,
      next
      ...
      if (!pub.getOption("fieldNameRegExp").test(fieldName)) {
        pub.util.logWarning(
          "Can not create connection field with improper name: '" +
            fieldName +
            "'"
        );
        next(
    ...
    connectionObj.setPresence = function (presenceObj, next) {
      if (e.app[appName].connection[easyrtcid]) {
        ...
        if (presenceObj.status !== undefined) {
          e.app[appName].connection[easyrtcid].presence.status =
            presenceObj.status;
        }
    ...
    connectionObj.emitRoomDataDelta = function (isLeavingAllRooms, callback) {
      pub.util.logDebug(
        "[" +
          ...
          "][" +
          easyrtcid +
          "] Running func 'connectionObj.emitRoomDataDelta'"
      );
      if (!_.isFunction(callback)) {
      ...
      connectionObj.generateRoomDataDelta(
        isLeavingAllRooms,
        function (err, newFullRoomDataDelta) {
            ...
            if (otherClients.hasOwnProperty(otherClient)) {
              ...
              for (var otherRoomName in otherClients[otherClient]) {
                if (otherClients[otherClient].hasOwnProperty(otherRoomName)) {
                  if (fullRoomDataDelta[otherRoomName]) {
                    msg.msgData.roomData[otherRoomName] =
                      fullRoomDataDelta[otherRoomName];
                  }
                }
              }
              ...
              (function (innerCurrentEasyrtcid, innerMsg) {
                connectionObj
                  .getApp()
                  .connection(
                    innerCurrentEasyrtcid,
                    function (err, emitToConnectionObj) {
                      if (
                        !err &&
                        innerCurrentEasyrtcid !== easyrtcid &&
                        emitToConnectionObj
                      ) {
                        pub.events.emit(
                          "emitEasyrtcCmd",
     ...
     */
    connectionObj.generateRoomClientList = function (
      roomStatus,
      roomMap,
      callback
    ) {
      ...
      for (var currentRoomName in e.app[appName].connection[easyrtcid].room) {
        if (
          e.app[appName].connection[easyrtcid].room.hasOwnProperty(
            currentRoomName
          )
        ) {
          ...
          var connectionRoom =
            e.app[appName].connection[easyrtcid].room[currentRoomName];
          ...
          for (var currentEasyrtcid in connectionRoom.toRoom.clientList) {
            if (
              connectionRoom.toRoom.clientList.hasOwnProperty(currentEasyrtcid)
            ) {
              var currentToConnection =
                connectionRoom.toRoom.clientList[currentEasyrtcid].toConnection;
              ...
              roomData[currentRoomName].clientList[currentEasyrtcid] = {
                ...
                roomJoinTime:
                  currentToConnection.room[currentRoomName].enteredOn,
              ...
              if (
                ...
                roomData[currentRoomName].clientList[
                  currentEasyrtcid
                ].apiField = currentToConnection.room[currentRoomName].apiField;
              }

              if (currentToConnection.username) {
                roomData[currentRoomName].clientList[
                  currentEasyrtcid
                ].username = currentToConnection.username;
              }
            }
          }
          ...
          for (var fieldName in connectionRoom.toRoom.field) {
            if (connectionRoom.toRoom.field.hasOwnProperty(fieldName)) {
              if (
                _.isObject(
                  connectionRoom.toRoom.field[fieldName].fieldOption
                ) &&
                ...
                roomData[currentRoomName].field[fieldName] = {
                  fieldName: fieldName,
                  fieldValue: pub.util.deepCopy(
                    connectionRoom.toRoom.field[fieldName].fieldValue
                  ),
                };
              }
            }
    ...
    connectionObj.generateRoomDataDelta = function (isLeavingRoom, callback) {
      if (!e.app[appName].connection[easyrtcid]) {
        ...
        callback(
          new pub.util.ConnectionWarning(
            "Attempt to request non-existent connection key: '" +
              easyrtcid +
              "'"
          )
        );
      ...
      pub.util.logDebug(
        "[" +
          ...
          "][" +
          easyrtcid +
          "] Running func 'connectionObj.generateRoomDataDelta'"
      );
        ...
        if (
          ...
          if (isLeavingRoom) {
            ...
            if (!_.isEmpty(e.app[appName].connection[easyrtcid].apiField)) {
              roomDataDelta[currentRoomName].clientListDelta.updateClient[
                easyrtcid
              ].apiField = e.app[appName].connection[easyrtcid].apiField;
            }
            if (e.app[appName].connection[easyrtcid].username) {
              roomDataDelta[currentRoomName].clientListDelta.updateClient[
                easyrtcid
              ].username = e.app[appName].connection[easyrtcid].username;
            }
          }
        }
      }
    ...
    connectionObj.generateRoomList = function (callback) {
      pub.util.logDebug(
        "[" +
          ...
          "][" +
          easyrtcid +
          "] Running func 'connectionObj.generateRoomList'"
      );
      var roomList = {};
      ...
      for (var currentRoomName in e.app[appName].room) {
        if (e.app[appName].room.hasOwnProperty(currentRoomName)) {
          roomList[currentRoomName] = {
            roomName: currentRoomName,
            numberClients: _.size(
              e.app[appName].room[currentRoomName].clientList
            ),
          };
        }
      }
    ...
    connectionObj.joinRoom = function (roomName, callback) {
      if (!e.app[appName].connection[easyrtcid]) {
        ...
        callback(
          new pub.util.ConnectionWarning(
            "Can not enter room as zombie: '" + roomName + "'"
          )
        );
        return;
      }
      if (!roomName || !appObj.getOption("roomNameRegExp").test(roomName)) {
        pub.util.logWarning(
          "[" +
            ...
            "][" +
            easyrtcid +
            "] Can not enter room with improper name: '" +
            roomName +
            "'"
        );
        callback(
          new pub.util.ConnectionWarning(
            "Can not enter room with improper name: '" + roomName + "'"
          )
        );
        return;
      }
      ...
      if (!appObj.isRoomSync(roomName)) {
        pub.util.logWarning(
          "[" +
            ...
            "][" +
            easyrtcid +
            "] Can not enter room which doesn't exist: '" +
            roomName +
            "'"
        );
        callback(
          new pub.util.ConnectionWarning(
            "Can not enter room which doesn't exist: '" + roomName + "'"
          )
        );
        return;
      }
    ...
    connectionObj.room = function (roomName, callback) {
      if (!e.app[appName].connection[easyrtcid]) {
        ...
        callback(
          new pub.util.ConnectionWarning(
            "Attempt to request non-existent room name: '" + roomName + "'"
          )
        );
        return;
      }
      ...
      connectionRoomObj.emitRoomDataDelta = function (isLeavingRoom, callback) {
        pub.util.logDebug(
          "[" +
            ...
            "] Room [" +
            roomName +
            "] Running func 'connectionRoomObj.emitRoomDataDelta'"
        );
        if (!_.isFunction(callback)) {
          callback = function (err, roomDataObj) {};
        }

        connectionRoomObj.generateRoomDataDelta(
          isLeavingRoom,
          function (err, roomDataDelta) {
            ...
            if (!appObj.isRoomSync(roomName)) {
              ...
              callback(
                new pub.util.ApplicationWarning(
                  "Attempt to request non-existent room name: '" +
                    roomName +
                    "'"
                )
              );
            ...
            for (var currentEasyrtcid in e.app[appName].room[roomName]
              .clientList) {
              if (
                e.app[appName].room[roomName].clientList.hasOwnProperty(
                  currentEasyrtcid
                )
              ) {
                // Anonymous wrapper to deliver arguments
       ...
       * @param       {function(?Error, Object=)} callback Callback with error and room data delta object.
       */
      connectionRoomObj.generateRoomDataDelta = function (
        isLeavingRoom,
        callback
      ) {
        pub.util.logDebug(
          "[" +
            ...
            "] Room [" +
            roomName +
            "] Running func 'connectionRoomObj.generateRoomDataDelta'"
        );
        if (!_.isFunction(callback)) {
        ...
        if (isLeavingRoom) {
          ...
          roomDataDelta.clientListDelta.removeClient[easyrtcid] = {
            easyrtcid: easyrtcid,
          };
        } else {
          var connectionRoom =
          ...
          roomDataDelta.clientListDelta.updateClient[easyrtcid] = {
            easyrtcid: easyrtcid,
            roomJoinTime:
              e.app[appName].connection[easyrtcid].room[roomName].enteredOn,
            presence: e.app[appName].connection[easyrtcid].presence,
          };

          if (
            !_.isEmpty(
              e.app[appName].connection[easyrtcid].room[roomName].apiField
            )
          ) {
            roomDataDelta.clientListDelta.updateClient[easyrtcid].apiField =
              e.app[appName].connection[easyrtcid].room[roomName].apiField;
          }
          if (e.app[appName].connection[easyrtcid].username) {
            roomDataDelta.clientListDelta.updateClient[easyrtcid].username =
              e.app[appName].connection[easyrtcid].username;
          }
        }
  ...
  appObj.createConnection = function (easyrtcid, socketId, callback) {
    if (!easyrtcid || !appObj.getOption("easyrtcidRegExp").test(easyrtcid)) {
      pub.util.logWarning(
        "Can not create connection with improper name: '" + easyrtcid + "'"
      );
      callback(
        new pub.util.ConnectionWarning(
          "Can not create connection with improper name: '" + easyrtcid + "'"
    ...
    if (e.app[appName].connection[easyrtcid]) {
      pub.util.logWarning(
        "Can not create connection which already exists: '" + easyrtcid + "'"
      );
      callback(
        new pub.util.ConnectionWarning(
          "Can not create connection which already exists: '" + easyrtcid + "'"
    ...
    appObj.connection(easyrtcid, function (err, connectionObj) {
      ...
      var connectionDefaultFieldObj = appObj.getOption(
        "connectionDefaultFieldObj"
      );
  ...
  appObj.createRoom = function (roomName, options, callback) {
    if (!roomName || !appObj.getOption("roomNameRegExp").test(roomName)) {
      pub.util.logWarning(
        "Can not create room with improper name: '" + roomName + "'"
      );
      callback(
        new pub.util.ApplicationWarning(
          "Can not create room with improper name: '" + roomName + "'"
    ...
    if (appObj.isRoomSync(roomName)) {
      pub.util.logWarning(
        "Can not create room which already exists: '" + roomName + "'"
      );
      callback(
        new pub.util.ApplicationWarning(
          "Can not create room which already exists: '" + roomName + "'"
    ...
    pub.util.logDebug(
      "Creating room: '" + roomName + "' with options:",
      options
    );
      ...
      async.each(
        ...
        function (err) {
          if (err) {
            callback(
              new pub.util.ApplicationError(
                "Could not set options when creating room: '" + roomName + "'",
                err
              )
            );
            return;
  ...
  appObj.createSession = function (easyrtcsid, callback) {
    pub.util.logDebug(
      "[" + appObj.getAppName() + "] Creating session [" + easyrtcsid + "]"
    );

    if (!easyrtcsid || !appObj.getOption("easyrtcsidRegExp").test(easyrtcsid)) {
      pub.util.logWarning(
        "Can not create session with improper name [" + easyrtcsid + "]"
      );
      callback(
        new pub.util.ConnectionWarning(
          "Can not create session with improper name [" + easyrtcsid + "]"
    ...
    if (e.app[appName].session[easyrtcsid]) {
      pub.util.logWarning(
        "Can not create session which already exists [" + easyrtcsid + "]"
      );
      callback(
        new pub.util.ConnectionWarning(
          "Can not create session which already exists [" + easyrtcsid + "]"
  ...
  appObj.isRoom = function (roomName, callback) {
    callback(
      ...
      e.app[appName] &&
        e.app[appName].room[roomName] &&
        !e.app[appName].room[roomName].deleted
        ? true
        : false
    );
  };
  ...
  appObj.group = function (groupName, callback) {
    if (!e.app[appName].group[groupName]) {
      pub.util.logWarning(
        "Attempt to request non-existent group name: '" + groupName + "'"
      );
      callback(
        new pub.util.ApplicationWarning(
          "Attempt to request non-existent group name: '" + groupName + "'"
    ...
    groupObj.getConnections = function (callback) {
      var connectedEasyrtcidArray = Object.keys(
        e.app[appName].group[groupName].clientList
      );
  ...
  appObj.room = function (roomName, callback) {
              ...
              function (err, targetConnectionObj) {
                if (err || !_.isObject(targetConnectionObj)) {
                  pub.util.logDebug(
                    "[" +
                      currentEasyrtcid +
                      "] Could not get connection object to send room data field update. Client may have disconnected."
                  );
                  asyncCallback(null);
                  return;
                }
    ...
    roomObj.getField = function (fieldName, callback) {
      ...
      if (!e.app[appName].room[roomName].field[fieldName]) {
        ...
        callback(
          new pub.util.ApplicationWarning(
            "Can not find room field: '" + fieldName + "'"
          )
        );
        return;
      }
      callback(
        null,
        pub.util.deepCopy(e.app[appName].room[roomName].field[fieldName])
      );
    };
    ...
    roomObj.getFieldValueSync = function (fieldName) {
      ...
      return pub.util.deepCopy(
        e.app[appName].room[roomName].field[fieldName].fieldValue
      );
    };
    ...
    roomObj.getFields = function (limitToIsShared, callback) {
      ...
      for (var fieldName in e.app[appName].room[roomName].field) {
        if (
          !limitToIsShared ||
          e.app[appName].room[roomName].field[fieldName].fieldOption.isShared
        ) {
          fieldObj[fieldName] = {
            fieldName: fieldName,
            fieldValue: pub.util.deepCopy(
    ...
    roomObj.getOption = function (optionName) {
      return !appObj.isRoomSync(roomName) ||
        e.app[appName].room[roomName].option[optionName] === undefined
        ? appObj.getOption(optionName)
        : e.app[appName].room[roomName].option[optionName];
    };
    ...
    roomObj.setField = function (fieldName, fieldValue, fieldOption, next) {
      ...
      if (!pub.getOption("fieldNameRegExp").test(fieldName)) {
        pub.util.logWarning(
          "Can not create room field with improper name: '" + fieldName + "'"
        );
        next(
          new pub.util.ApplicationWarning(
            "Can not create room field with improper name: '" + fieldName + "'"
    ...
    roomObj.getConnections = function (callback) {
      ...
      var connectedEasyrtcidArray = Object.keys(
        e.app[appName].room[roomName].clientList
      );
      callback(null, connectedEasyrtcidArray);
    };
      ...
      if (e.app[appName].room[roomName].clientList[easyrtcid]) {
        appObj.connection(easyrtcid, function (err, connectionObj) {
          if (err) {
            callback(
              new pub.util.ConnectionWarning(
                "Can not find connection [" + easyrtcid + "] in room."
              )
            );
            return;
          }
  ...
  appObj.session = function (easyrtcsid, callback) {
    if (!e.app[appName].session[easyrtcsid]) {
      pub.util.logWarning(
        "Attempt to request non-existent easyrtcsid: '" + easyrtcsid + "'"
      );
      callback(
        new pub.util.ApplicationWarning(
          "Attempt to request non-existent easyrtcsid: '" + easyrtcsid + "'"
    ...
    sessionObj.getField = function (fieldName, callback) {
      if (!e.app[appName].session[easyrtcsid].field[fieldName]) {
        ...
        callback(
          new pub.util.ApplicationWarning(
            "Can not find session field: '" + fieldName + "'"
          )
        );
        return;
      }
      callback(
        null,
        pub.util.deepCopy(e.app[appName].session[easyrtcsid].field[fieldName])
      );
    };
    ...
    sessionObj.getFieldValueSync = function (fieldName) {
      ...
      return pub.util.deepCopy(
        e.app[appName].session[easyrtcsid].field[fieldName].fieldValue
      );
    };
    ...
    sessionObj.getFields = function (limitToIsShared, callback) {
      ...
      for (var fieldName in e.app[appName].session[easyrtcsid].field) {
        if (
          !limitToIsShared ||
          e.app[appName].session[easyrtcsid].field[fieldName].fieldOption
            .isShared
        ) {
          fieldObj[fieldName] = {
            fieldName: fieldName,
    ...
    sessionObj.setField = function (fieldName, fieldValue, fieldOption, next) {
      ...
      if (!pub.getOption("fieldNameRegExp").test(fieldName)) {
        pub.util.logWarning(
          "Can not create session field with improper name: '" + fieldName + "'"
        );
        next(
          new pub.util.ApplicationWarning(
            "Can not create session field with improper name: '" +
              fieldName +
              "'"
          )
        );
              ...
              function (err, targetConnectionObj) {
                if (err || !_.isObject(targetConnectionObj)) {
                  pub.util.logDebug(
                    "[" +
                      currentEasyrtcid +
                      "] Could not get connection object to send session data field update. Client may have disconnected."
                  );
                  asyncCallback(null);
                  return;
                }
